import json
from config_execution_api import mode, api_key, api_secret, api_url
...
def POST(endPoint, body:json):
    ...
    print('result POST', api_url + endPoint)
    # requests.post('https://httpbin.org/post', data=payload)

    response = httpClient.request("POST", api_url+endPoint ,headers=headers, data=body)
    print(response)

    response = requests.post( api_url+endPoint, headers=headers, json=body)
    print(response)
